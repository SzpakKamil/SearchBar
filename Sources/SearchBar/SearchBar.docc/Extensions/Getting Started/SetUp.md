...
The `SearchBar` package provides a customizable SwiftUI search bar for iOS, and macOS. It leverages native components (`UISearchBar` for iOS/visionOS, `NSSearchField` for macOS) with a SwiftUI API, supporting features like custom styling, accessibility (VoiceOver and Dynamic Type), and platform-specific behaviors. This guide explains how to add the package using Swift Package Manager (SPM).
...
struct ContentView: View {
    ...
    var body: some View {
        SearchBar(text: $searchText, prompt: "Search")
            .searchBarOnSearchPerform { print("Searching: \(searchText)") }
    }
}
```
